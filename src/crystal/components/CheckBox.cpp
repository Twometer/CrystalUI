...
namespace crystal {
    ...
    void CheckBox::draw(Graphics &graphics) {
        ...
        graphics.push_rectangle(position.x + 1, position.y + 1, size.y - 2, size.y - 2,
                                glm::vec3(0.1529, 0.1922, 0.2118));

        checkContentColor.update();
        graphics.push_rectangle(position.x + 8, position.y + 8, size.y - 16, size.y - 16,
                                checkContentColor.get_value());
        graphics.end();
